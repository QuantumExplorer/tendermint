...
func TestBlockAddEvidence(t *testing.T) {
	...
	block := MakeBlock(h, chainLock, txs, commit, evList)
...
func TestBlockHash(t *testing.T) {
	...
	assert.Nil(t, MakeBlock(int64(3), NewMockChainLock(), []Tx{Tx("Hello World")}, nil, nil).Hash())
}

func TestBlockMakePartSet(t *testing.T) {
	assert.Nil(t, (*Block)(nil).MakePartSet(2))

	partSet := MakeBlock(int64(3), NewMockChainLock(), []Tx{Tx("Hello World")}, nil, nil).MakePartSet(1024)
...
func TestBlockMakePartSetWithEvidence(t *testing.T) {
	...
	partSet := MakeBlock(h, chainLock, []Tx{Tx("Hello World")}, commit, evList).MakePartSet(512)
	assert.NotNil(t, partSet)
...
func TestBlockHashesTo(t *testing.T) {
	...
	block := MakeBlock(h, chainLock, []Tx{Tx("Hello World")}, commit, evList)
...
func TestBlockSize(t *testing.T) {
	size := MakeBlock(int64(3), NewMockChainLock(), []Tx{Tx("Hello World")}, nil, nil).Size()
	if size <= 0 {
...
func TestBlockString(t *testing.T) {
	...
	block := MakeBlock(int64(3), NewMockChainLock(), []Tx{Tx("Hello World")}, nil, nil)
...
func TestBlockMaxDataBytes(t *testing.T) {
	testCases := []struct {
		...
		2: {982, crypto.Ed25519,1, 0, true, 0},
		3: {984, crypto.Ed25519,1, 0, false, 0},
		4: {985, crypto.Ed25519,1, 0, false, 1},
		5: {-10, crypto.BLS12381, 1, 0, true, 0},
		6: {10, crypto.BLS12381,1, 0, true, 0},
		7: {1014, crypto.BLS12381,1, 0, true, 0},
		8: {1016, crypto.BLS12381,1, 0, false, 0},
		9: {1017, crypto.BLS12381,1, 0, false, 1},
	}
...
func TestBlockProtoBuf(t *testing.T) {
	...
	b1 := MakeBlock(h, chainLock, []Tx{Tx([]byte{1})}, &Commit{Signatures: []CommitSig{}}, []Evidence{})
	b1.ProposerAddress = tmrand.Bytes(crypto.AddressSize)

	b2 := MakeBlock(h, chainLock, []Tx{Tx([]byte{1})}, c1, []Evidence{})
	...
	b3 := MakeBlock(h, chainLock, []Tx{}, c1, []Evidence{})
